module uim.domains.attributes.attribute;
...
class DAttribute : DEntity {
 ...
 /*  O attribute(this O)(UUID myId, size_t myMajor = 0, size_t myMinor = 0) { 
    _attribute = OOPAttribute.id(myId).versionMajor(myMajor).versionMinor(myMinor);
    return cast(O)this; }

  O attribute(this O)(string myName, size_t myMajor = 0, size_t myMinor = 0) { 
    _attribute = OOPAttribute.name(myName).versionMajor(myMajor).versionMinor(myMinor);
    return cast(O)this; }
...
@safe auto OOPAttribute() { return new DAttribute; }
@safe auto OOPAttribute(UUID id) { return new DAttribute(id); }
@safe auto OOPAttribute(string name) { return new DAttribute(name); }
@safe auto OOPAttribute(UUID id, string name) { return new DAttribute(id, name); }
@safe auto OOPAttribute(Json json) { return new DAttribute(json); }
@safe auto OOPAttribute(DETBCollection aCollection, Json json) { return (new DAttribute(json)).collection(aCollection); }